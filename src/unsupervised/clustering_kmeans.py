...
### Unsupervised - Clustering (spherical k-means)
### By Edgar Daniel
...
Spherical k-means clustering pipeline for cross-year comparison of a
count-valued outcome (``y_cnt``) conditional on cluster membership.
 
Design notes
------------
* Geometry: features are assumed to live in [-1, 1]^d without a unit-norm
  guarantee. We L2-normalize rows so that Euclidean distance on the sphere
  is a monotone function of cosine similarity (||u - v||^2 = 2 - 2cos(u,v)),
  which lets us run a *true* spherical k-means (centroid renormalization
  after every partial fit) using ``MiniBatchKMeans`` as the inner solver.
 
* Outcome variable: ``y_cnt`` is a non-negative integer count (event
  counts per spatial unit). We deliberately do NOT use KDE/violin plots
  for it: a Gaussian kernel places continuous density mass at
  non-integer and negative values. 
  Instead we report the empirical PMF and ECDF that both
  admit valid pointwise comparison across clusters (e.g. via
  Kolmogorov-Smirnov-style visual diagnostics on the ECDF panel).
 
* Effect size over significance: at n ~ 10^6, Kruskal-Wallis p-values
  collapse to numerical zero for essentially any nonzero effect. We
  therefore treat epsilon^2 = (H - k + 1) / (n - k) as the substantive
  quantity and p as a formality.
 
Usage
-----
    python -m src.unsupervised.clustering_kmeans --config config/kmeans_conf.yml

or programmatically:

    from src.unsupervised.clustering_kmeans import PipelineConfig, run_pipeline
...
# Allow both `python -m src.unsupervised.clustering_kmeans` and direct execution.
...
class DataConfig:
    ...
    key_column: str                        # per-entry identifier, preserved across years for traceability
...
@dataclass(frozen=True)
class ModelConfig:
    k_range: Sequence[int]                 # exploration sweep, e.g. list(range(2, 15))
    k_list: Sequence[int]                  # final k's to fit and compare, e.g. [4, 7]
    rng_seed: int = 69
...
@dataclass(frozen=True)
class PaletteConfig:
    """Parametrizable, minimal-ink palette.

    Defaults come from the shared house style (``src.utils.style``); every
    field remains overridable from the YAML ``viz.palette`` block.
    """
    base_color: str = HOUSE.navy            # deep navy accent (ramp source)
    highlight_color: str = HOUSE.bad        # red, reference lines/annotations only
    ink_color: str = HOUSE.ink              # text / axis labels
    axis_color: str = HOUSE.muted           # spines / tick labels
    grid_color: str = HOUSE.grid            # gridlines
    background_color: str = HOUSE.background
    font_family: str = HOUSE.font_family
    n_shades: int = 9                       # max categorical levels to pre-generate
    dpi: int = 220
    fig_format: str = "png"
    categorical_hex: Optional[Sequence[str]] = None  # explicit override, takes precedence
...
class PipelineConfig:
    ...
    def validate(self) -> None:
        if not set(self.model.k_list).issubset(set(self.model.k_range)):
            logger.warning("k_list %s not fully contained in k_range %s — sweep will not "
                            "cover every final k.", self.model.k_list, self.model.k_range)
        if len(self.data.feature_columns) == 0:
            raise ValueError("data.feature_columns is empty.")
        Path(self.viz.fig_path).mkdir(parents=True, exist_ok=True)
 


def apply_house_style(palette: PaletteConfig) -> None:
    """Sets a high data-ink-ratio matplotlib/seaborn theme: no chart
    junk, horizontal-only gridlines, desaturated ink, spines removed on
    the top/right. Idempotent — safe to call multiple times."""
...
# Fixed-order categorical set for cluster identity (PCA scatter, PMF/ECDF
# panels, stacked shares)
_CLUSTER_HEX: tuple[str, ...] = HOUSE.qual


def categorical_palette(palette: PaletteConfig, n: int) -> list[str]:
    """``n`` colors for distinguishing clusters: an explicit override via
    ``categorical_hex`` takes precedence; otherwise draws in fixed order
    from the house categorical set. Past that (rare), falls back to shading
    ``palette.base_color`` dark-to-light."""
    if palette.categorical_hex is not None:
        if len(palette.categorical_hex) < n:
            raise ValueError(
                f"categorical_hex has {len(palette.categorical_hex)} colors, "
                f"need {n}.")
...
def strip_axis(ax: plt.Axes) -> None:
    """Removes tick marks (keeping labels) — final touch for minimal-ink axes."""
...
def spherical_kmeans(
    ...
    """True spherical k-means: MiniBatchKMeans as the Lloyd-step engine,
    with centroids re-projected onto the unit sphere after every partial
    fit so that squared-Euclidean updates stay cosine-consistent."""
    ...
    for it in range(cfg.spherical_n_iter):
        ...
        if shift < cfg.spherical_tol:
            logger.debug("spherical_kmeans(k=%d) converged at iter %d (shift=%.2e)", k, it, shift)
            break
        prev = mbk.cluster_centers_.copy()
    return mbk
...
def run_k_sweep(X_norm: np.ndarray, year, cfg: ModelConfig) -> pd.DataFrame:
    """Model-selection sweep over cfg.k_range: inertia, Calinski-Harabasz,
    Davies-Bouldin (full data) and silhouette (subsampled for tractability)."""
    n = X_norm.shape[0]
    ...
    for k in cfg.k_range:
        ...
        records.append(rec)
        logger.info("[year=%s] k=%2d | inertia=%.1f | CH=%.1f | DB=%.3f | silhouette=%.3f",
...
def empirical_pmf(y: np.ndarray, support: np.ndarray) -> np.ndarray:
    """P(Y = v) for v in `support`, with values beyond support.max() folded
...
def empirical_ecdf(y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Right-continuous step ECDF: returns (sorted unique values, F(v))."""
...
def plot_discrete_distributions(
    ...
    """Paired diagnostic for a discrete outcome: empirical PMF (left,
    grouped bars, shared capped support with an overflow bucket) and
    ECDF (right, step lines) — both valid descriptors of a count
    distribution, unlike a KDE/violin overlay."""
    ...
    # PMF panel: grouped bars across clusters, dodge on x 
    ...
    for i, (c, color) in enumerate(zip(clusters, colors)):
        ...
        axes[0].bar(support + offset, pmf, width=width, color=color, label=f"cluster {c}")
    xticks = list(support)
    xticklabels = [str(v) for v in support[:-1]] + [f"\u2265{cap}"]
    axes[0].set_xticks(xticks)
    axes[0].set_xticklabels(xticklabels)
    axes[0].set_title(f"PMF — year {year}, k={k_star}")
    axes[0].set_xlabel(value_col)
    axes[0].set_ylabel("P(Y = y)")
    axes[0].legend(loc="upper right")
 
    # ECDF panel: one step line per cluster
    for c, color in zip(clusters, colors):
        y_c = df_year.loc[df_year[cluster_col] == c, value_col].to_numpy()
        vals, cdf = empirical_ecdf(y_c)
        vals_capped = np.minimum(vals, cap)
        axes[1].step(vals_capped, cdf, where="post", color=color, label=f"cluster {c}", linewidth=1.6)
    axes[1].set_xlim(support.min(), cap)
    axes[1].set_ylim(0, 1.02)
    axes[1].set_title(f"ECDF — year {year}, k={k_star}")
    axes[1].set_xlabel(f"{value_col} (capped at {cap})")
...
def plot_cluster_size_share(df_k: pd.DataFrame, k_star: int, viz: VizConfig) -> plt.Figure:
    """Stacked bar of cluster-size share by year — a legitimate use of a
    stacked bar since shares of a discrete partition (cluster
    membership) are exactly what stacking represents, unlike density
    overlays for continuous mass."""
    apply_house_style(viz.palette)
    ...
    ax.set_title(f"Cluster size share by year — k={k_star}")
...
def plot_sweep_diagnostics(sweep_df: pd.DataFrame, year, k_list: Sequence[int], viz: VizConfig) -> plt.Figure:
    ...
    panels = [
        ("inertia", "Inertia (WCSS) vs k", axes[0, 0]),
        ("silhouette", "Silhouette (subsample) vs k", axes[0, 1]),
...
def plot_pca_projection(
    ...
    for c, color in zip(clusters, colors):
        m = labels_2d == c
        ax.scatter(X_2d[m, 0], X_2d[m, 1], s=5, alpha=0.6, color=color, label=f"cluster {c}", linewidths=0)
    ax.legend(title="cluster", loc="best")
    ax.set_title(f"PCA — year {year}, k={k_star}")
...
def plot_effect_size_trend(eff_table: pd.DataFrame, k_star: int, viz: VizConfig) -> plt.Figure:
    ...
    ax.set_title(f"Cluster separation strength (\u03b5\u00b2) by year — k={k_star}")
    ax.set_xlabel("year")
    ax.set_ylabel("\u03b5\u00b2")
...
def kruskal_effect_size(df_k: pd.DataFrame, k_star: int, value_col: str) -> pd.DataFrame:
    """Kruskal-Wallis H and epsilon^2 = (H - k + 1) / (n - k), by year.
    Reported instead of relying on p (which saturates near zero at
    large n regardless of effect magnitude)."""
    rows = []
    for year, g in df_k.groupby("year"):
        groups = [gg[value_col].to_numpy() for _, gg in g.groupby("cluster")]
        if len(groups) < 2:
            logger.warning("year=%s has %d cluster(s); skipping Kruskal-Wallis.",
                           year, len(groups))
            continue
        try:
            H, p = stats.kruskal(*groups)
        except ValueError:
            # All values identical across groups: no separations to test.
            logger.warning("year=%s: identical %s values; skipping test.",
                           year, value_col)
            continue
...
def _savefig(fig: plt.Figure, viz: VizConfig, name: str) -> None:
    path = Path(viz.fig_path) / f"{name}.{viz.palette.fig_format}"
...
def run_pipeline(cfg: PipelineConfig, gdf_all: Optional[pd.DataFrame] = None) -> PipelineResult:
    """End-to-end: sweep -> fit -> discrete distribution diagnostics ->
    cross-year effect-size comparison. `gdf_all` can be injected directly
    (e.g. already loaded via geopandas) to keep this module agnostic of
    the geo I/O layer; otherwise it is read from cfg.data.parquet_path
    via pandas (parquet does not require a geo reader unless geometry is
    needed downstream)."""
    cfg.validate()
    ...
    # Stage 2: final fits per year x k 
    for year in years:
        gdf_y = gdf_all.query(f"{cfg.data.year_column} == @year").sort_values(cfg.data.sort_column)
        X_norm = load_normalized_features(gdf_y, cfg.data.feature_columns)
    ...
    # Stage 3: cross-year comparison per k
    for k_star in cfg.model.k_list:
    ...
    # cross-year cluster assignment map, keyed for downstream joins against
    # the original training set (year, key_column, k, cluster) ----
    ...
    if cfg.output.cluster_assignments_path:
        ...
        logger.info("saved cluster assignments -> %s", cfg.output.cluster_assignments_path)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Spherical k-means clustering pipeline.")
    ...
    result = run_pipeline(cfg)
    logger.info("Pipeline complete. Years x k combinations fitted: %d", len(result.models))
